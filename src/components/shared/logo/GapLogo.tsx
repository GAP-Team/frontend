...
export const GapLogo = ({
  size,
  color = "white",
  src,
}: GapLogoProps): JSX.Element => {
  const inlineStyles = {
  ...
  return (
    <div
      className={`flex flex-row justify-start items-center gap-4 p-8`}
      style={inlineStyles.textColor}
    >
      <div
        style={{
          position: "relative",
      ...
      >
        <Image priority alt="Follow us on Twitter" src={src ?? gapLogo} fill />
      </div>
      <span className={`${lalezar.className} ${textSize}  text-center md:mt-3`}>
        GAP
      </span>
      <div
        className={`inline-block ${dividerWidth} mx-[1px] h-12`}
        style={inlineStyles.dividerBg}
      />
