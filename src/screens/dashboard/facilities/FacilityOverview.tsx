...
const FacilityOverview: React.FC<FacilitiesProps> = ({ facilityId }): JSX.Element => {
  ...
  const facility = useMemo(() => 
    facilities.find((facility: Facility) => facility.id === facilityId), [facilities, facilityId]);

  const building = useMemo(() => 
    buildings.find((building: Building) => building.id === facility?.buildingId), [buildings, facility]);

  useEffect(() => {
    if (user?.id) {
      dispatch(fetchBuildings({ userId: user.id, city: "", federalState: "", facilityType: "" }));
      dispatch(getFacilitiesByUser(user.id, building?.city || "", building?.federalState || "", facility?.facilityType || ""));
    }
  }, [user?.id, dispatch, building, facility]);

  const onFilterCriteriaChange = useCallback(async (
    city: string,
    state: string,
    facilityType: string
  ): Promise<void> => {
    await dispatch(getFacilitiesByUser(user?.id, city, state, facilityType)).unwrap();
  }, [dispatch, user?.id]);

  const hasFacilities = useMemo(() => 
    filteredBuildings?.some((building: Building) => building?.facilityIds?.length > 0), [filteredBuildings]);
